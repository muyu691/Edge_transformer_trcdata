#!/bin/bash
# GPU Slurm launcher for the standalone NodeCentricGNN baseline on Vera.
#
# Default usage:
#   sbatch run_node_centric_gnn_vera_gpu.sh
#
# Switch dataset:
#   DATASET_NAME=siouxfalls sbatch run_node_centric_gnn_vera_gpu.sh

#SBATCH -J base_node
#SBATCH -o logs/base_node_%j.out
#SBATCH -e logs/base_node_%j.err
#SBATCH -t 0-12:00:00
#SBATCH -n 1
...
RUN_TAG="${RUN_TAG:-node_centric_gnn_${DATASET_NAME}}"
OUTPUT_DIR="${OUTPUT_DIR:-${OUTPUT_ROOT}/${RUN_TAG}}"

cd "${PROJECT_ROOT}"

echo "================ NodeCentricGNN (Vera GPU) ================"
