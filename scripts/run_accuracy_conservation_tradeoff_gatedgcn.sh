#!/bin/bash
# Slurm array for the accuracy-conservation trade-off of the ST-PINN GatedGCN model.
#
# Default usage from the project root:
#   sbatch scripts/run_accuracy_conservation_tradeoff_gatedgcn.sh
#
# Run only one experiment:
#   sbatch --array=3 scripts/run_accuracy_conservation_tradeoff_gatedgcn.sh

#SBATCH -J ac_tradeoff
#SBATCH -o logs/ac_tradeoff_%A_%a.out
#SBATCH -e logs/ac_tradeoff_%A_%a.err
#SBATCH -t 0-12:00:00
#SBATCH -n 1
...
if [[ ! -f "${PROJECT_ROOT}/run_topology_common.sh" ]]; then
  ...
  echo "Submit this script from /cephyr/users/wuxin/Vera/Physics-Informed_Diffusion_Model-main/Network_reconfiguration-main or set PROJECT_ROOT explicitly." >&2
  exit 1
fi
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/results/accuracy_conservation_tradeoff}"
...
RUN_TAG="stpinn_gatedgcn_${DATASET_NAME}_lambda_con_${SAFE_LAMBDA}_10000"
RESOLVED_DATASET_DIR="${DATASET_DIR}"

if [[ ! -d "${RESOLVED_DATASET_DIR}" ]]; then
  echo "Dataset directory does not exist: ${RESOLVED_DATASET_DIR}" >&2
  exit 1
fi

mkdir -p "${PROJECT_ROOT}/logs" "${OUTPUT_ROOT}"

echo "============== Accuracy-Conservation Trade-Off =============="
