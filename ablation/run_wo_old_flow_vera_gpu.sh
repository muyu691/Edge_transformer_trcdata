#!/bin/bash
# GPU Slurm launcher for the Edge Transformer w/o old-flow ablation on Vera.

#SBATCH -J abl_wof
#SBATCH -o logs/abl_wof_%j.out
#SBATCH -e logs/abl_wof_%j.err
#SBATCH -t 0-12:00:00
#SBATCH -n 1
...
RUN_TAG="${RUN_TAG:-ablation_wo_old_flow_${DATASET_NAME}_10000}"
...
echo "============= Edge Transformer W/O Old Flow (Vera GPU) ============="
...
srun python main.py \
  ...
  topology_gnn.alignment_mode "wo_old_flow" \
  topology_gnn.initial_flow_mode "zeros" \
  topology_gnn.initial_pressure_mode "zeros" \
