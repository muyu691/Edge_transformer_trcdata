#!/bin/bash
# Slurm array for the Edge Transformer depth sweep on Vera.
#
# Tasks:
#   0-1: SiouxFalls num_edge_transformer_layers in {2, 3}
#   2-3: EMA        num_edge_transformer_layers in {2, 3}
#   4-5: Anaheim    num_edge_transformer_layers in {2, 3}
# The depth=1 row is the full Edge Transformer model.

#SBATCH -J et_depth
#SBATCH -o logs/et_depth_%A_%a.out
#SBATCH -e logs/et_depth_%A_%a.err
#SBATCH -t 0-12:00:00
#SBATCH -n 1
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/results/edge_transformer_depth_sweep}"
...
DATASETS=(siouxfalls siouxfalls ema ema anaheim anaheim)
DEPTH_VALUES=(2 3 2 3 2 3)
...
EDGE_TRANSFORMER_LAYERS="${DEPTH_VALUES[$TASK_ID]}"

set_topology_main_hparams
RESOLVED_DATASET_DIR="$(resolve_topology_dataset_dir)"
NETWORK_NAME="$(resolve_topology_network_name)"
RUN_TAG="${RUN_TAG:-edge_transformer_${DATASET_NAME}_depth_${EDGE_TRANSFORMER_LAYERS}_10000}"
...
srun python main.py \
  ...
  topology_gnn.num_edge_transformer_layers "${EDGE_TRANSFORMER_LAYERS}" \
