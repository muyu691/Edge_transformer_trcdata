#!/bin/bash
# Slurm array for the Edge Transformer pseudo-time diffusion-step sweep on Vera.
#
# Tasks:
#   0-2: SiouxFalls num_diffusion_steps in {2, 4, 6}
#   3-5: EMA        num_diffusion_steps in {2, 4, 6}
#   6-8: Anaheim    num_diffusion_steps in {2, 4, 6}
# The num_diffusion_steps=4 rows are identical to the full Edge Transformer model.

#SBATCH -J et_steps
#SBATCH -o logs/et_steps_%A_%a.out
#SBATCH -e logs/et_steps_%A_%a.err
#SBATCH -t 0-12:00:00
#SBATCH -n 1
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/results/edge_transformer_diffusion_steps_sweep}"
...
DATASETS=(siouxfalls siouxfalls siouxfalls ema ema ema anaheim anaheim anaheim)
DIFFUSION_STEP_VALUES=(2 4 6 2 4 6 2 4 6)
...
DIFFUSION_STEPS="${DIFFUSION_STEP_VALUES[$TASK_ID]}"

set_topology_main_hparams
RESOLVED_DATASET_DIR="$(resolve_topology_dataset_dir)"
NETWORK_NAME="$(resolve_topology_network_name)"
RUN_TAG="${RUN_TAG:-edge_transformer_${DATASET_NAME}_diffusion_steps_${DIFFUSION_STEPS}_10000}"
...
echo "============== Edge Transformer diffusion-step sweep =============="
...
srun python main.py \
  ...
  topology_gnn.num_diffusion_steps "${DIFFUSION_STEPS}" \
