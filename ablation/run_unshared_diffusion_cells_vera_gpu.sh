#!/bin/bash
# GPU Slurm launcher for the Edge Transformer unshared-diffusion-cells ablation on Vera.
# Parameter count is logged by main.py; wall-clock training time is START_TIME -> END_TIME.

#SBATCH -J abl_unshr
#SBATCH -o logs/abl_unshr_%j.out
#SBATCH -e logs/abl_unshr_%j.err
#SBATCH -t 0-12:00:00
#SBATCH -n 1
...
RUN_TAG="${RUN_TAG:-ablation_unshared_diffusion_cells_${DATASET_NAME}_10000}"
...
echo "======= Edge Transformer Unshared Diffusion Cells (Vera GPU) ======="
...
srun python main.py \
  ...
  topology_gnn.share_diffusion_cell False \
