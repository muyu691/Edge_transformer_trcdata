#!/bin/bash
# GPU Slurm launcher for the standalone single-topology GatedGCN baseline on Vera.
#
# Default usage:
#   sbatch run_single_topology_gatedgcn_vera_gpu.sh
#
# Switch dataset:
#   DATASET_NAME=siouxfalls sbatch run_single_topology_gatedgcn_vera_gpu.sh

#SBATCH -J base_gated
#SBATCH -o logs/base_gated_%j.out
#SBATCH -e logs/base_gated_%j.err
#SBATCH -t 0-12:00:00
#SBATCH -n 1
...
RUN_TAG="${RUN_TAG:-single_topology_gatedgcn_${DATASET_NAME}}"
OUTPUT_DIR="${OUTPUT_DIR:-${OUTPUT_ROOT}/${RUN_TAG}}"

cd "${PROJECT_ROOT}"

echo "============ Single-Topology GatedGCN (Vera GPU) ============"
...
srun python -m baseline.run \
  --model single_topology_gatedgcn \
  "${DATASET_ARGS[@]}" \
