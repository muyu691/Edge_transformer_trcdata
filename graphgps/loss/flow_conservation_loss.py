...
<<<<<<< HEAD
Phase 5 adds a non-redundant rerouting residual auxiliary objective:

    Delta_f* = f_new - f_init
    L_total  = L_data + lambda_eq * L_eq + lambda_rc * L_rc
               + lambda_reroute_residual * L_reroute_residual
...
<<<<<<< HEAD
def _require_batch_tensor(batch, name: str, branch: str = 'Physics branch') -> torch.Tensor:
=======
...
>>>>>>> 058655b286e2606488e7fb1e8072052dd458d355
    ...
    if value is None:
        raise RuntimeError(
<<<<<<< HEAD
            f"{branch} requires batch.{name}, but it is missing. "
            f"Please ensure topology_model.forward() attaches the final-step "
            f"auxiliary tensors before compute_pinn_loss()."
=======
...
<<<<<<< HEAD
    reduced_cost_final = _require_batch_tensor(batch, 'reduced_cost_final', branch='Reduced-cost branch')
    phi_v_final = _require_batch_tensor(batch, 'phi_v_final', branch='Reduced-cost branch')
    f_active_final = _require_batch_tensor(batch, 'f_active_final', branch='Reduced-cost branch')
=======
...
<<<<<<< HEAD
def _compute_rerouting_residual_loss(batch, reference: torch.Tensor) -> torch.Tensor:
    """Auxiliary Delta-f supervision on a dedicated residual head.

    This branch is intentionally non-redundant: the model exposes a separate
    ``reroute_residual_pred_scaled`` tensor instead of simply rewriting the
    main final-flow loss on ``pred - f_init``.
    """
    lambda_reroute_residual = float(getattr(cfg.model, 'lambda_reroute_residual', 0.0))
    if lambda_reroute_residual <= 0.0:
        return _zero_loss(reference)

    reroute_residual_pred = _require_batch_tensor(
        batch,
        'reroute_residual_pred_scaled',
        branch='Rerouting residual branch',
    )
    f_init_scaled = _require_batch_tensor(
        batch,
        'f_init_scaled',
        branch='Rerouting residual branch',
    )
    true_delta = batch.y - f_init_scaled
    reroute_residual_pred, true_delta = _align_pred_true_shapes(
        reroute_residual_pred,
        true_delta,
    )

    _require_finite_tensor('batch.reroute_residual_pred_scaled', reroute_residual_pred)
    _require_finite_tensor('batch.f_init_scaled', f_init_scaled)
    _require_finite_tensor('rerouting residual target', true_delta)
    return _compute_data_loss(reroute_residual_pred, true_delta)
...
<<<<<<< HEAD
    lambda_reroute_residual = float(getattr(cfg.model, 'lambda_reroute_residual', 0.0))
    loss_reroute_residual = _compute_rerouting_residual_loss(
        batch=batch,
        reference=pred,
    )
    total_loss = (
        ...
        + lambda_eq * loss_eq
        + lambda_rc * loss_rc
        + lambda_reroute_residual * loss_reroute_residual
    )
=======
...
<<<<<<< HEAD
    batch.loss_reroute_residual = loss_reroute_residual.detach()
=======
>>>>>>> 058655b286e2606488e7fb1e8072052dd458d355
